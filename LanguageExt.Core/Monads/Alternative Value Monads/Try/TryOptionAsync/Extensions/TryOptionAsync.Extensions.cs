...
public static partial class TryOptionAsyncExtensions
{
    ...
    /// <returns>Mapped computation</returns>
    [Pure]
    public static TryOptionAsync<B> BiMap<A, B>(this TryOptionAsync<A> self, Func<A, B> Succ, Func<Error, Error> Fail) => 
        Memo(async () =>
    {
        var res = await self.Try().ConfigureAwait(false);
        return res switch
        {
            {IsSome:true} x=> Succ(x.Value.Value),
            {IsNone:true} => OptionalResult<B>.None,
            var x=> new OptionalResult<B>(Fail(x.Exception))
        };
    });
    ...
    /// <returns>Mapped computation</returns>
    [Pure]
    public static TryOptionAsync<B> BiMapAsync<A, B>(this TryOptionAsync<A> self, Func<A, ValueTask<B>> Succ, Func<Error, ValueTask<Error>> Fail) => 
        Memo(async () =>
        {
            var res = await self.Try().ConfigureAwait(false);
            return res switch
            {
                {IsSome:true} x=> await Succ(x.Value.Value).ConfigureAwait(false),
                {IsNone:true} => OptionalResult<B>.None,
                var x=> new OptionalResult<B>(await Fail(x.Exception).ConfigureAwait(false))
            };
        });
    ...
    public static TryOptionAsync<D> Join<A, B, C, D>(
        ...
        Func<A, B, D> project) =>
            Memo<D>(async () =>
            {
                ...
                await WaitAsync.All(selfTask, innerTask).ConfigureAwait(false);
    ...
    public static async Task<int> Compare<ORD, A>(this TryOptionAsync<A> lhs, TryOptionAsync<A> rhs) where ORD : struct, Ord<A> 
    {
        var x = lhs.Try();
        var y = rhs.Try();
        await WaitAsync.All(x, y).ConfigureAwait(false);
